  	...
  	if ( isset( $_POST['tdUploadEventPrimForm_nonce'] ) && wp_verify_nonce( $_POST['tdUploadEventPrimForm_nonce'], 'tdUploadEventPrimForm_html' ) ) {
		...
		if($response == 1) {
			...
			if(isset($_POST['numberOfSeats'])) { 
				$number_of_seats = sanitize_text_field($_POST['numberOfSeats']);
				update_post_meta($post_id, 'numberOfSeats', $number_of_seats);
			}

			if(isset($_POST['seatsComment'])) { 
				$seats_comment = sanitize_text_field($_POST['seatsComment']);
				update_post_meta($post_id, 'seatsComment', $seats_comment);
			}
			...
			//Speaker's Name
			if(isset($_POST['listing-upload-speakers-image-data'])) { $item_speaker_name = $_POST['listing-upload-speakers-image-data']; };
			update_post_meta($post_id,'item-speaker', $item_speaker);

			//Speaker's Info
			if(isset($_POST['listing-upload-speakers-image-data'])) { $item_speaker_about = $_POST['listing-upload-speakers-image-data']; };
			update_post_meta($post_id,'item-speaker', $item_speaker_about);

			//Speaker's Link
			if(isset($_POST['listing-upload-speakers-image-data'])) { $item_speaker_link = $_POST['listing-upload-speakers-image-data']; };
			update_post_meta($post_id,'item-speaker', $item_speaker_link);

			//Speaker Small Info
			if(isset($_POST['speakerComment'])) { $speaker_comment = sanitize_text_field($_POST['speakerComment']);
				update_post_meta($post_id, 'speakerComment', $speaker_comment);}

			// Partners
			if(isset($_POST['listing-upload-partners-image-data'])) { $item_partners = $_POST['listing-upload-partners-image-data']; };
			update_post_meta($post_id,'item-partners', $item_partners);

			//Schedule
			if(isset($_POST['listing-upload-schedule'])) { $item_schedule = $_POST['listing-upload-schedule']; };
			update_post_meta($post_id, 'item-schedule', $item_schedule);

			if(isset($_POST['scheduleComment'])) { $schedule_comment = sanitize_text_field($_POST['scheduleComment']);
				update_post_meta($post_id, 'scheduleComment', $schedule_comment);}
